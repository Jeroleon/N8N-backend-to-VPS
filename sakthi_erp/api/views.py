...
def update_product_details(request, product_id):
    try:
        ...
        prod.programer_status = request.data.get("programer_status", prod.programer_status)
        ...
        for mat_data in materials_data:
            ...
            mat.programer_status = mat_data.get("programer_status", mat.programer_status)
            mat.qa_status = mat_data.get("qa_status", mat.qa_status)
            mat.acc_status = mat_data.get("acc_status", mat.acc_status)
            mat.save()
        
        return Response({"msg": "Product and materials updated successfully"}, status=status.HTTP_200_OK)
    except Exception as e:
        return Response({"msg": f"Error updating product: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def delete_product(request, product_id):
    try:
        ...
        return Response({"msg": f"Error deleting product: {str(e)}"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    try:
        ...
        try:
            prog = programer_details.objects.get(
                ...
                material_details_id=material_id
            )
        ...
        fields_to_update = [
            "program_no", "program_date", "processed_quantity", "balance_quantity",
            "processed_width", "processed_length", "used_weight", "number_of_sheets",
            "cut_length_per_sheet", "pierce_per_sheet", "processed_mins_per_sheet",
            "total_planned_hours", "total_meters", "total_piercing",
            "total_used_weight", "total_no_of_sheets", "date", "time", "remarks"
        ]
...
#delete 
@api_view(["DELETE"])
def delete_programer(request, programer_id):
    try:
        programer = programer_details.objects.get(id=programer_id)
        programer.delete()
        return Response({"msg": "Programmer deleted successfully"}, status=200)
    except programer_details.DoesNotExist:
        return Response({"msg": "Programmer not found"}, status=404)
    except Exception as e:
        return Response({"msg": f"Error deleting programmer: {str(e)}"}, status=500)
...
def update_qa_details(request, product_id):
    try:
        ...
        # Update fields
        fields = [
            "processed_date", "time", "shift", "no_of_sheets",
            "cycletime_per_sheet", "total_cycle_time",
            "machines_used", "operator_name"
        ]
...
def delete_qa(request, qa_id):
    try:
        qa = Qa_details.objects.get(id=qa_id)
        qa.delete()
        return Response({"msg": "QA deleted successfully"}, status=200)
    except Qa_details.DoesNotExist:
        return Response({"msg": "QA not found"}, status=404)
    except Exception as e:
        return Response({"msg": f"Error deleting QA: {str(e)}"}, status=500)
...
def add_acc_details(request):
    try:
        ...
        for material_id in material_ids:
            try:
                material_instance = product_material.objects.get(id=material_id, product_detail=prod)
            except product_material.DoesNotExist:
                return Response({
                    "status": False,
...
@api_view(["PUT"])
def update_acc_details(request, product_id):
    try:
        material_id = request.data.get("material_details")
        ...
        # Ensure material belongs to this product
        try:
            product_material.objects.get(id=material_id, product_detail_id=product_id)
        except product_material.DoesNotExist:
            return Response({"msg": "Material does not belong to this product"}, status=400)

        # Get correct account entry
        try:
            acc = acc_details.objects.get(
                product_details_id=product_id,
                material_details_id=material_id
            )
        except acc_details.DoesNotExist:
            return Response({"msg": "Account details not found"}, status=404)

        # Update fields
        if "date" in request.data:
            acc.date = request.data["date"]

        if "time" in request.data:
            acc.time = request.data["time"]

        if "invoice_no" in request.data:
            acc.invoice_no = request.data["invoice_no"]

        if "status" in request.data:
            acc.status = request.data["status"]

        if "remarks" in request.data:
            acc.remarks = request.data["remarks"]

        # Update created_by (ForeignKey)
        created_by = request.data.get("created_by")
        if created_by:
            try:
                acc.created_by = All_User.objects.get(username=created_by)
            except All_User.DoesNotExist:
                return Response({"msg": "created_by user not found"}, status=400)

        acc.save()

        return Response({"msg": "Account details updated successfully"}, status=200)

    except Exception as e:
        return Response({"msg": f"Error updating account details: {str(e)}"}, status=500)

@api_view(["DELETE"])
def delete_acc(request, acc_id):
    try:
        acc = acc_details.objects.get(id=acc_id)
        acc.delete()
        return Response({"msg": "Account deleted successfully"}, status=200)
    except acc_details.DoesNotExist:
        return Response({"msg": "Account not found"}, status=404)
    except Exception as e:
        return Response({"msg": f"Error deleting account: {str(e)}"}, status=500)
            ...
            for mat in materials:
                ...
                acc_info = list(
                    acc_details.objects.filter(material_details=mat).values(
                        "id", "invoice_no", "date", "time", "status", "remarks", "created_by__username"
                    )
                )
...
def export_specific_details(request, ids=None):
    ...
    try:
        ...
        headers = [
            "Product ID", "Company", "Serial No", "Customer Name",
            "Material ID", "Mat Type", "Mat Grade", "Thickness",
            "Width", "Length", "Quantity", "Total Weight", "Bay",
            "Programmer Status", "QA Status", "Account Status",
            "Program No", "Processed Qty", "Balance Qty", "Used Weight",
            "Sheets Count", "Program Created By", "QA Processed Date",
            "QA Shift", "QA Created By", "ACC Invoice No",
            "ACC Status", "ACC Created By",
        ]
        ws.append(headers)
        ...
        for prod in products_qs:
            ...
            for mat in materials:
                ...
                ws.append([
                    prod.id, prod.company_name, prod.serial_number, prod.customer_name,
                    mat.id, mat.mat_type, mat.mat_grade, mat.thick,
                    mat.width, mat.length, mat.quantity, mat.total_weight,
                    mat.bay, mat.programer_status, mat.qa_status, mat.acc_status,
                    prog.program_no if prog else None,
                    prog.processed_quantity if prog else None,
                    prog.balance_quantity if prog else None,
                    prog.used_weight if prog else None,
        ...
        response["Content-Disposition"] = 'attachment; filename="overall_details_export.xlsx"'
